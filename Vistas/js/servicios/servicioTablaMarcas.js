 ...
 */
 ...
 const crearTablaMarcas = (res, filas) => {
    ...
    for (const elementos of res) {
      ...
      celdaidentificador.innerHTML = elementos.idmarca ;
      celdadescripcion.innerHTML = elementos.descripcionm;
  ...
  const crearTablaVaciaMarcas = (filas) => {
      const fragment = document.createDocumentFragment();
  ...
  const crearTablaErrorMarcas = (filas) => {
      const fragment = document.createDocumentFragment();
  ...
  const cargarMarcas = async (fila) => {  
    let elemento = "todos";
    try {
        const url = `Controladores/ApiFetchTablas.php?idmarcas=${elemento}`;
        const res = await fetch(url);
        const data = await res.json();
        if(data.length === 0){
              crearTablaVaciaMarcas(fila);
        }else{
            crearTablaMarcas(data, fila);
        }
    } catch (error) {
        console.log(error);
      crearTablaErrorMarcas(fila);
    }
  }
