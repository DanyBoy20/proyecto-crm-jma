 ...
 */
 ...
 const crearSelectPolizas = (dato, capacitacion) => {
    const fragment = document.createDocumentFragment();
      for (const serviciopolizas of dato) {
        const option = document.createElement("OPTION");
        option.setAttribute("value", serviciopolizas.idtipopoliza);
        option.textContent = serviciopolizas.descripcion;
        fragment.append(option);
      }      
    capacitacion.appendChild(fragment);
  }

 const cargaPolizas = async (elemento) => {   
    try {
      let valor = "todos";
      const url = `Controladores/ApiFetchPolizas.php?tipo=${valor}`;
      const res = await fetch(url);
      const data = await res.json();
      crearSelectPolizas(data, elemento);
    } catch (err) {
      console.log(err);    
    }
  }
...
const cargarIdElementoHospital = (event, elementoid, elementodescripcion, solicitante, area) => {
  if(event.inputType == "insertReplacementText" || event.inputType == null) {
    elementoid.value =  event.target.value;
    let arreglo = event.target.list.children;
    for(let i = 0; i< arreglo.length; i++){
      if(event.target.value == event.target.list.children[i].value){
        ...
        cargarAreas(elementoid.value, area);
        break;
      }
    }
  }
