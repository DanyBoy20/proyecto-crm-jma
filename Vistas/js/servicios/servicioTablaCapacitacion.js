 ...
 */
 ...
 const crearTablaCapacitacion = (res, filas) => {
    ...
    for (const elementos of res) {
      ...
      celdaidentificador.innerHTML = elementos.idtipocapacitaciones;
  ...
  const crearTablaVaciaCapacitacion = (filas) => {
      const fragment = document.createDocumentFragment();
  ...
  const crearTablaErrorCapacitacion = (filas) => {
      const fragment = document.createDocumentFragment();
  ...
  const cargarCapacitaciones = async (fila) => {  
    let elemento = "todos";
    try {
        const url = `Controladores/ApiFetchTablas.php?capacitaciones=${elemento}`;
        const res = await fetch(url);
        const data = await res.json();
        if(data.length === 0){
              crearTablaVaciaCapacitacion(fila);
        }else{
            crearTablaCapacitacion(data, fila);
        }
    } catch (error) {
        console.log(error);
      crearTablaErrorCapacitacion(fila);
    }
  }
  
   export const servicioCapacitacion = {
